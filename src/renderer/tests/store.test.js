...
class StoreTestRunner {
  ...
  setLastSetValue(delegate) {
    return (this.lastSetValue = delegate(this.lastSetValue));
  }
}

function runStoreTest(initialState) {
  const runner = new StoreTestRunner(initialState);
  const store = storeLogic(runner.intercomApi)((v) =>
    runner.setLastSetValue(v),
  );
...
const TEST_DOCUMENT_DATA_A = { name: 'SHEET_1', data: [{ col_a: 'A' }] };
const TEST_DOCUMENT_DATA_B = { name: 'SHEET_1', data: [{ col_b: 'B' }] };
...
test('showTermsAndConditions', () => {
  [SCREEN_LOAD_NEW_CONFIG, SCREEN_CONFIG_UPDATED].forEach((screen) => {
    const { store, runner } = runStoreTest({ screen });
...
test('startConfigChange', () => {
  const result = testStoreCall(TEST_STATE, (store) =>
    store.startConfigChange(),
  );
...
test('Intercom userConfigRemoved error', () => {
  const now = new Date();
  const result = testStoreCall(TEST_STATE, (store) =>
    store.userConfigRemoved({ success: false, error: TEST_ERROR }),
  );
...
test('updateConfig', () => {
  ...
  const result = testStoreCall(TEST_STATE, (store) =>
    store.updateConfig(TEST_NEW_CONFIG, IS_BACKUP),
  );
...
test('Intercom startPreProcessingFile', () => {
  ...
  expect(runner.intercomApi.calls).toEqual([
    ['startPreProcessingFile', TEST_PATH],
  ]);
...
test('Intercom preProcessingDone OK', () => {
  const result = testStoreCall(TEST_STATE, (store) => {
    return store.preProcessingDone({
      ...
      document: TEST_DOCUMENT_DATA_A,
    });
  });
  expect(!TEST_VALIDATION_RESULT_OK.some((sheet) => !sheet.ok)).not.toEqual(
    undefined,
  );
  ...
  expect(result.document).toEqual(TEST_DOCUMENT_DATA_A);
...
test('Intercom preProcessingDone ERROR', () => {
  ...
  expect(result.document).toEqual(TEST_DOCUMENT_DATA_B);
...
test('Intercom startProcessingFile', () => {
  ...
  expect(runner.intercomApi.calls).toEqual([
    ['startProcessingFile', TEST_PATH, TEST_PATH_B],
  ]);
