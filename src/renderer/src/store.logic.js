...
export function storeLogic(intercomApi) {
  return (set) => {
    return {
      screen: SCREEN_BOOT,
      ...
      updateConfig: (newConfig, isBackup) =>
        set((state) => {
          return {
            ...state,
            config: {
      ...
      loadNewConfigDone: ({ success, canceled, error, config, lastUpdated }) =>
        set((state) => {
      ...
      boot: ({
        ...
        set((state) => {
          ...
          const targetScreen = hasAcceptedTermsAndConditions
            ? SCREEN_MAIN
            : SCREEN_WELCOME;
          return {
            config: {
      ...
      // output format
      startProcessingFile: (inputFilePath, outputLocation) =>
        set((state) => {
