...
describe('config actions', () => {
  ...
  it('loadNewConfig calls IPC and ends in CONFIG_UPDATED (success)', async () => {
    vi.spyOn(bridge, 'loadNewConfig').mockResolvedValue({
      status: "success",
      config: { meta: { id: 'X', version: '1', signature: '' } } as unknown as Config.FileConfiguration,
  ...
  it('loadNewConfig: sets LOAD_NEW_CONFIG before IPC resolves', async () => {
    ...
    let resolve!: (v?: ILoadNewConfig) => void;
    const promise = new Promise((r) => (resolve = r));
    vi.spyOn(bridge, 'loadNewConfig').mockReturnValue(promise as unknown as Promise<ILoadNewConfig>);
    ...
    resolve({
      status: "success",
      config: { meta: { id: 'X', version: '1', signature: '' } } as any,
      lastUpdated: new Date()
    });
    await promiseCall;
  ...
  it('removeConfig calls IPC and updates backup state', async () => {
    vi.spyOn(bridge, 'removeConfig').mockResolvedValue({
      success: true, config: { meta: { id: 'B', version: '1', signature: '' } }, lastUpdated: new Date()
    } as any);

    await configAction.removeConfig();

    expect(bridge.removeConfig).toHaveBeenCalled();
    expect(getState().screen).toBe(SCREENS.CONFIG_UPDATED);
    expect(getState().config.isBackup).toBe(true);
  });

  it('removeConfig: sets LOAD_NEW_CONFIG before IPC resolves', async () => {
    let resolve!: (v?: unknown) => void;
    const promise = new Promise((r) => (resolve = r));
    vi.spyOn(bridge, 'removeConfig').mockReturnValue(promise as any);

    const promiseCall = configAction.removeConfig();
    expect(getState().screen).toBe(SCREENS.LOAD_NEW_CONFIG);
