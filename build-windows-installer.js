...
function findFirstDirectoryMatchingPattern(baseDir, rxPattern) {
  ...
  if (dirs.length === 0) {
    throw new Error(
      `Unable to find any directory in '${baseDir}' matching the pattern /${rxPattern}/`,
    );
  }
...
function buildSignToolCommand(basePath = '') {
  function getSigntoolPath() {
    ...
    const sdkDir = path.join(
      fullBasePath,
      findFirstDirectoryMatchingPattern(
        fullBasePath,
        /Microsoft\.Windows\.SDK\.BuildTools/,
      ),
    );
...
async function runBuild() {
  ...
  try {
    ...
    const windowsSignOpts = buildSignToolCommand(
      process.platform == 'darwin' ? '/tmp/' : 'C:\\',
    );
    const buildOptions = {
